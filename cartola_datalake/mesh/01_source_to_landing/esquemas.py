# python -m cartola_datalake.mesh.01_source_to_landing.esquemas
import os
...
_log = SetupLogger("01_source_to_landing.esquemas")
...
def main():
    ...
    path_esquemas = os.path.join(
        os.getcwd(),
        "datalake",
        FOLDER_LANDING,
        f"season-{SEASON_STR}",
        "esquemas",
        f"{now}_esquemas.json",
    )
    ...
    if list_jsons:
        ...
        if hash_url in dict_hash:
            _log.info(
                f"esquemas | no new data found... same hash/info of '{dict_hash[hash_url]}'",
            )
            return
