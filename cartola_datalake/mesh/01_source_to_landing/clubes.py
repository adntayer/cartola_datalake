# uv run -m cartola_datalake.mesh.01_source_to_landing.clubes
import os
...
def main():
    ...
    path_clubes = os.path.join(os.getcwd(), 'datalake', FOLDER_LANDING, f'season-{SEASON_STR}', 'clubes', f'{now}_clubes.json')
    ...
    if list_jsons:
        ...
        if hash_url in dict_hash:
            _log.info(f"clubes | no new data found... same hash/info of '{dict_hash[hash_url]}'")
            return
