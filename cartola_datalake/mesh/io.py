...
def write_if_different(df, filepath, **to_csv_kwargs):
    ...
    _log.info(f'write_if_different | {os.path.basename(filepath)} | New hash: {new_hash}')
    if old_hash:
        _log.info(f'write_if_different | {os.path.basename(filepath)} | Old hash: {old_hash}')
    else:
        _log.info(f"write_if_different | {os.path.basename(filepath)} | Old file doesn't exist, so no previous hash to compare.")

    if new_hash != old_hash or not old_hash:
        if os.path.exists(filepath):
            os.remove(filepath)
            _log.info(f'write_if_different | {os.path.basename(filepath)} | Deleted old file: {filepath}')

        df.to_csv(filepath, **to_csv_kwargs)
        _log.info(f'write_if_different | {os.path.basename(filepath)} | Created updated file: {filepath}')
    else:
        _log.info(f'write_if_different | {os.path.basename(filepath)} | No changes in file: {filepath}')
